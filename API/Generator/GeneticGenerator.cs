...
namespace API.Generator
{
    public class GeneticGenerator
    {
        ...
        public List<FitnessSchedule> Population { get; set; }
        public Dictionary<Group, List<WorkloadTeachers>> Workload { get; set; }

        public GeneticGenerator(Dictionary<Group, List<WorkloadTeachers>> workload, int populationSize, int numberOfGenerations, double mutationRate, Schedule schedule)
        {
            _schedule = schedule;
            ...
            this.Population = new List<FitnessSchedule>();
        }
        ...
        public List<Lesson> RunGeneticAlgorithm(int threads)
        {
            ...
            {
                BlockingCollection<FitnessSchedule> newPopulation = new BlockingCollection<FitnessSchedule>();
                ...
                Population = new((FitnessSchedule[])newPopulation.ToArray());
        ...
        public List<FitnessSchedule> GenPopulation(int count)
        {
            List<FitnessSchedule> schedules = new List<FitnessSchedule>();
            for (int i = 0; i < count; i++)
            {
                FitnessSchedule child = Crossover(SelectParent(), SelectParent());
                //FitnessSchedule child = SelectParent();
                Mutate(child);
        ...
        private FitnessSchedule SelectParent()
        {
            var top = Population.OrderByDescending(s => s.Fitness).Take(populationSize / 10).ToList();
            // Простой турнирный отбор
            FitnessSchedule parent1 = top[random.Next(top.Count)];
            FitnessSchedule parent2 = Population[random.Next(Population.Count)];
        ...
        public void InitializePopulation()
        {
            ...
            {
                FitnessSchedule schedule = CreateRandomSchedule();
                CalculateFitness(schedule);
                Population.Add(schedule);
            }
        }

        private FitnessSchedule CreateRandomSchedule()
        {
        ...
        private void CalculateFitness(FitnessSchedule schedule)
        {
            int fitness = 0;
        ...
        public FitnessSchedule Crossover(FitnessSchedule parent1, FitnessSchedule parent2)
        {
            var childSchedule = new FitnessSchedule();
        ...
        public void Mutate(FitnessSchedule schedule)
        {
